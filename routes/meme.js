...
// Indonesian meme from 1cak
router.get("/api/meme/1cak", asyncHandler(async (req, res) => {
  const meme = await memeAgg.get1cakMeme();
  res.json({
    success: true,
    source: "1cak",
    data: meme
...
router.post("/api/meme/1cak", asyncHandler(async (req, res) => {
  const meme = await memeAgg.get1cakMeme();
  res.json({
    success: true,
    source: "1cak",
...
router.get("/api/meme/reddit/:subreddit", asyncHandler(async (req, res) => {
  ...
  if (!subreddit || subreddit === ':subreddit') {
    return res.status(400).json({
      success: false,
      error: "Please provide a valid subreddit name",
      example: "/api/meme/reddit/dankmemes"
    });
  }
  
  const meme = await memeAgg.getRedditMeme(subreddit);
  res.json({
    success: true,
    source: "reddit",
    subreddit,
...
export const metadata = [
  ...
  {
    name: "Reddit Subreddit Meme",
    path: "/api/meme/reddit/:subreddit",
    method: "GET",
    description: "Get meme from specific subreddit",
    params: [
      {
        name: "subreddit",
        type: "text",
        required: true,
        placeholder: "dankmemes",
        description: "Subreddit name (replace :subreddit in URL)"
      }
    ]
  },
  {
  ...
  {
    name: "1cak Indonesian Meme",
    path: "/api/meme/1cak",
    method: "GET, POST",
    description: "Get random Indonesian meme from 1cak.com",
    params: []
  }
];
