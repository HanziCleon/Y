...
const WerewolfChatGame = () => {
  ...
  const advancedBotChat = (botName, phase, currentGameState) => {
    ...
    if (message) {
      setTimeout(async () => {
        ...
        if (binId) {
          await updateBin(binId, { gameState, messages: updatedMessages });
        }
  ...
  const resetGameAfterWin = async () => {
    ...
    if (binId) {
      await updateBin(binId, { gameState: newGameState, messages: [] });
    }
  };
  ...
  const addBot = async () => {
    ...
    if (binId) {
      await updateBin(binId, { gameState: newGameState, messages: updatedMessages });
    }
  ...
  const createRoom = async () => {
    ...
    if (newBinId) {
      ...
      showNotif(`Room ${code} dibuat! BinID: ${newBinId}`, 'success');
    }
  };
  ...
  const startGame = async () => {
    ...
    const updatedMessages = [systemMsg];
    setMessages(updatedMessages);
    setGameState(newGameState);
  ...
  const nextPhase = async () => {
    ...
    if (binId) {
      await updateBin(binId, { gameState: newGameState, messages: newMessages });
    }
  };
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col text-white overflow-hidden relative">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
      
      {showNotification && (
        <div className={`fixed top-6 left-1/2 transform -translate-x-1/2 px-8 py-4 rounded-2xl font-black z-50 shadow-2xl max-w-sm w-full mx-4 text-center backdrop-blur-xl border-2 animate-bounce ${
      ...
      {/* Header */}
      <div className="bg-black/80 backdrop-blur-xl border-b-2 border-purple-500 p-4 sticky top-0 z-20 shadow-2xl">
      ...
      {/* Chat Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24 relative z-10">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.type === 'system' || msg.type === 'story' ? 'justify-center' : msg.user === username ? 'justify-end' : 'justify-start'}`}>
            {msg.type === 'system' ? (
              <div className="max-w-[90%]">
                ...
                <div className="bg-gradient-to-r from-yellow-900/70 to-orange-900/70 backdrop-blur-sm border-2 border-yellow-500 rounded-2xl px-5 py-3 shadow-xl">
                  <p className="text-sm font-bold text-yellow-200 text-center">{msg.text}</p>
                </div>
              </div>
            ) : msg.type === 'story' ? (
              <div className="bg-gradient-to-r from-purple-900/70 to-pink-900/70 backdrop-blur-sm border-2 border-purple-500 rounded-2xl px-5 py-3 max-w-[90%] shadow-xl">
                <p className="text-sm font-bold text-purple-200 text-center">{msg.text}</p>
              </div>
            ) : (
              <div className={`max-w-[80%] ${msg.user === username ? 'bg-gradient-to-r from-purple-600 to-pink-600' : 'bg-gradient-to-r from-gray-800 to-gray-700'} rounded-2xl p-4 shadow-xl border-2 ${msg.user === username ? 'border-purple-400' : 'border-gray-600'} transform hover:scale-105 transition-all`}>
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center font-black text-sm">
                    {msg.user[0]}
                  </div>
                  ...
                  <span className="text-[10px] text-gray-400 ml-auto">{new Date(msg.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <p className="break-words text-white text-sm font-medium">{msg.text}</p>
              </div>
            )}
          </div>
        ))}
      ...
      {/* Input Area */}
      <div className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-xl border-t-4 border-purple-500 p-4 shadow-2xl z-20">
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-5 py-4 bg-gradient-to-r from-purple-900/50 to-pink-900/50 border-2 border-purple-500 rounded-full text-white focus:outline-none focus:border-pink-400 font-bold text-sm placeholder-purple-300 transition-all"
          />
          <button onClick={sendMessage} className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 rounded-full hover:from-purple-700 hover:to-pink-700 active:scale-95 transition-all shadow-xl flex items-center justify-center">
