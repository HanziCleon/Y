...
const WerewolfChatGame = () => {
    ...
    loadGoogleScript().then(() => {
      ...
      if (savedGoogleUser) {
        try {
          ...
          // Auto-reconnect to room if exists
  ...
  }, [isLoggedIn, googleButtonRef.current]);
  ...
  const handleReady = async () => {
    ...
    const systemMsg = {
      id: Date.now(),
      user: 'Desa',
      text: `✅ ${roomUsername} siap bermain (${newReady.length}/${gameState.players.length})`,
      timestamp: new Date().toISOString(),
      type: 'system'
    };
      ...
      <div className="flex-shrink-0 bg-slate-900/90 backdrop-blur-xl border-b border-blue-500/30 p-3 sm:p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {roomCode && (
              <button onClick={copyRoomCode} className="flex-shrink-0 items-center gap-1 bg-slate-800/50 border border-blue-500/30 rounded-lg px-2 sm:px-3 py-1 sm:py-2 font-bold text-xs active:scale-95 transition-all">
